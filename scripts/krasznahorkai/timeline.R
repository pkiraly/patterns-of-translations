...
# df2 %>% filter(lang == 'angol') %>% arrange(title) %>% view()
df2 %>% 
  count(year, is_first)

df2 %>% 
  count(year, is_first) %>% 
  ggplot(aes(x = year, y = n, fill = is_first)) +
  geom_col() +
  theme_minimal() +
  labs(title = "Krasznahorkai idegen nyelven",
       x = "megjelenési év",
       y = "kiadványszám",
       fill = "") +
  scale_fill_manual(
    labels = c("újrakiadás", "új fordítás"),
    # values = c(TRUE, FALSE)
    values = c("cornflowerblue", "maroon")
    ) +
  scale_x_continuous(
...
ggsave('images/krasznahorkai/timeline.hu.png',
...
df2 %>% 
  ...
  labs(title = "",
       ...
       fill = "") +
  scale_fill_manual(
    labels = c("reprint", "new translation"),
    # values = c(TRUE, FALSE)
    values = c("cornflowerblue", "maroon")
  ) +
  scale_x_continuous(
...
ggsave('images/krasznahorkai/timeline.en.png',
       width = 12, height = 4, units = 'in', dpi = 300)
...
ggsave('images/krasznahorkai13.en.png',
...
ggsave('images/krasznahorkai13.hu.png',
       width = 12, height = 8, units = 'in', dpi = 300)
